...
async function main() {
  ...
  botUsername = tokens.WICKRIO_BOT_NAME.value
  botPort = tokens.botPort.value
  botApiKey = tokens.botApiKey.value
  botApiAuthToken = tokens.botApiAuthToken.value
  const httpsChoice = tokens.HTTPS_CHOICE.value

  console.log('botUsername=' + botUsername)
  console.log('botPort=' + botPort)
  console.log('httpsChoice=' + httpsChoice)

  if (httpsChoice === 'yes' || httpsChoice === 'y') {
    sslKeyLocation = tokens.sslKeyLocation.value
    sslCrtLocation = tokens.sslCrtLocation.value

    try {
      if (!fs.existsSync(sslKeyLocation)) {
        exitHandler(null, {
          exit: true,
          reason: 'ERROR: Cannot access ' + sslKeyLocation,
        })
      }
    } catch (err) {
      console.error(err)
    ...
    try {
      if (!fs.existsSync(sslCrtLocation)) {
        exitHandler(null, {
          exit: true,
          reason: 'ERROR: Cannot access ' + sslCrtLocation,
        })
      }
    } catch (err) {
      console.error(err)
    ...
    const credentials = {
      key: fs.readFileSync(sslKeyLocation, 'utf8'),
      cert: fs.readFileSync(sslCrtLocation, 'utf8'),
    }

    https.createServer(credentials, app).listen(botPort, () => {
      console.log('HTTPS Server running on port', botPort)
    })
  } else {
    app.listen(botPort, () => {
      console.log('We are live on ' + botPort)
    })
  }
  ...
  const endpoint = '/WickrIO/V1/Apps/' + botApiKey
    ...
    if (!checkCreds(authToken)) {
      ...
      if (reason === 'leave') {
        try {
          ...
          res.send(botUsername + ' left room successfully')
        } catch (err) {
          console.log(err)
          res.statusCode = 400
          res.type('txt').send(err.toString())
  ...
  app.delete(endpoint + '/GroupConvo/:vGroupID', function (req, res) {
    ...
    if (!checkCreds(authToken)) {
      ...
      try {
        ...
        console.log(cdgc)
        res.send(botUsername + ' has left the GroupConvo.')
      } catch (err) {
        console.log(err)
        res.statusCode = 400
        res.type('txt').send(err.toString())
...
function checkCreds(authToken) {
  try {
    ...
    if (authStr !== botApiAuthToken) valid = false
    return valid
  } catch (err) {
    console.log(err)
  }
