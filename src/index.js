...
async function main() {
  ...
  const https_choice = tokens.HTTPS_CHOICE.value
  ...
  app.delete(endpoint + '/Messages/:vGroupID/:messageID', function (req, res) {
    ...
    const doRecall = req.query.dorecall ? req.query.dorecall : 'false'
    if (doRecall === 'true') {
      try {
        const cdr = WickrIOAPI.cmdSendRecallMessage(vGroupID, msgID)
      ...
      try {
        const clr = WickrIOAPI.cmdSendDeleteMessage(vGroupID, msgID)
